...
from models import SetGNN, HCHA, HNHN, HyperGCN, HyperSAGE, \
    LEGCN, UniGCNII, HyperND, EquivSetGNN, PlainMLP, PlainUnigencoder, Unigencoder
...
def main(args):
    ...
    elif args.method in ['PlainUnigencoder', 'Unigencoder']:
        data = PlainUnigencoder.d_expansion(data, args.threshold, args.norm_type, args.init_val, args.init_type, args.sample_rate)
    data = data.to(device)
    ...
    elif args.method == 'PlainMLP':
        model = PlainMLP(data.num_features, args.hidden, data.num_classes, args.num_layers, args.dropout)
    elif args.method == 'PlainUnigencoder':
        model = PlainUnigencoder(data.num_features, args.hidden, data.num_classes, args.num_layers, args.dropout)
    else:
    ...
    test_mean, test_std = logger.get_best_res()
    row = [args.method, args.dname, args.num_layers, args.lr, args.hidden, args.dropout,
           args.weight_decay, args.init_val, args.init_type, args.sample_rate, test_mean, test_std]
    with open("output_23_5_9_{}_{}_hy.csv".format(args.dname, str(args.method)), mode="a", newline="") as file:
...
def obj(trial, args):
    ...
    threshold = trial.suggest_categorical('threshold', [-5])
    # init_val = trial.suggest_categorical('init_val', [1, 10, 100, 0.1, 0.001, 0.0001])
    # init_type = trial.suggest_categorical('init_type', [1, 0])
    init_val = trial.suggest_categorical('init_val', [1])
    init_type = trial.suggest_categorical('init_type', [0])
    sample_rate = trial.suggest_categorical('sample_rate', [0.1])
    # model definition
    print(lr, hidden, num_layers, weight_decay, dropout, threshold, norm_type, init_val, init_type, sample_rate)

    args.weight_decay, args.num_layers, args.lr, args.hidden, \
    args.dropout, args.threshold, args.norm_type, \
    args.init_val, args.init_type, args.sample_rate = weight_decay, num_layers, lr, \
                                                      hidden, dropout, threshold, norm_type, init_val, init_type, sample_rate
...
if __name__ == '__main__':
    ...
    # Args for Unigencoder
    parser.add_argument('--hidden', default=64, type=int, help='hidden dimension of mlps')
    parser.add_argument('--num_layers', default=2, type=int, help='number of basic blocks')
    ...
    # main(args)
    print('start training {} {}'.format(args.method, args.dname))
    study = optuna.create_study(direction='maximize')
    # study.optimize(obj, n_trials=10)
    study.optimize(lambda trial: obj(trial, args), n_trials=100)
    ...
    args.sample_rate = best_params['sample_rate']
    acc_row = [args.method, args.dname, args.num_layers, args.lr,
               args.hidden, args.weight_decay, args.dropout, args.threshold,
               args.norm_type, args.init_val, args.init_type, args.sample_rate, study.best_trial.value]
    print(acc_row)
    with open("mean_23_5_9_{}_{}.csv".format(args.dname, args.method), mode="a", newline="") as file:
        # Create a CSV writer object
        writer = csv.writer(file)
        # Write the data to the CSV file row by row
